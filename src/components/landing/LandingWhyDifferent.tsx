  ...
  return (
    <section className="py-16 sm:py-28">
      <div className="max-w-5xl mx-auto px-5 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px rounded-xl overflow-hidden bg-[hsl(var(--ivory-dim)/0.08)] mb-12 sm:mb-16">
          <FadeInView className="bg-[hsl(var(--obsidian-light)/0.4)] p-6 sm:p-10">
            <span className="text-xs font-mono tracking-[0.15em] text-[hsl(var(--ivory-dim)/0.5)] mb-6 block">WITHOUT A SYSTEM</span>
            <div className="space-y-3">
              {BEFORE.map((item, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="h-1.5 w-1.5 rounded-full bg-[hsl(var(--ivory-dim)/0.25)] shrink-0" />
                  <span className="text-sm text-[hsl(var(--ivory-dim)/0.5)] line-through decoration-[hsl(var(--ivory-dim)/0.2)]">{item}</span>
                </div>
              ))}
            </div>
          </FadeInView>

          <FadeInView delay={0.1} className="bg-[hsl(var(--obsidian-light)/0.2)] p-6 sm:p-10 relative">
            ...
            <div className="space-y-3 relative">
              {AFTER.map((item, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-[hsl(var(--ivory)/0.85)] font-medium">{item}</span>
                </div>
              ))}
            </div>
          </FadeInView>
        ...
        <FadeInView className="max-w-3xl mx-auto text-center py-8 sm:py-12 border-y border-[hsl(var(--gold-oxide)/0.12)]">
          <p className="text-[hsl(var(--ivory))] font-serif font-bold text-lg sm:text-xl leading-snug mb-4">
            AI-IDEI is built around one goal: help you turn thought into execution.
          </p>
          <p className="text-sm text-[hsl(var(--ivory-dim)/0.5)] font-mono tracking-[0.1em]">
