...
const TYPE_PATH: Record<string, string> = {
  ...
  application: "applications", contradiction: "contradictions",
  profile: "profiles", topic: "topics",
};

const TYPE_LABEL: Record<string, string> = {
  insight: "Insight", pattern: "Pattern", formula: "Formula",
  application: "Application", contradiction: "Contradiction",
  profile: "Profile", topic: "Topic",
};

const RELATION_LABEL: Record<string, string> = {
  DERIVED_FROM: "Derived from", RELATES_TO: "Related to", APPLIES_TO: "Applies to",
  CONTRADICTS: "Contradicts", MENTIONS: "Mentions", PART_OF: "Part of",
  INSPIRES: "Inspires", SUPPORTS: "Supports", EXTENDS: "Extends", REFERENCES: "References",
};

const FAMILY_LABEL: Record<string, string> = {
  decision: "Decision Intelligence", strategy: "Strategic Intelligence",
  economic: "Economic Intelligence", behavioral: "Behavioral Intelligence",
  cognitive: "Cognitive Intelligence", system: "Systems Intelligence",
  knowledge: "Knowledge Intelligence", communication: "Communication Intelligence",
  organization: "Organization Intelligence", innovation: "Innovation Intelligence",
  risk: "Risk Intelligence", meta: "Meta Intelligence",
};
...
export default function EntityDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: ent } = await supabase
        .from("entities").select("*").eq("slug", slug).eq("is_published", true).single();
      ...
      const [outRels, inRels, topicsRes] = await Promise.all([
        supabase.from("entity_relations").select("id, relation_type, weight, target_entity_id")
          .eq("source_entity_id", ent.id).order("weight", { ascending: false }).limit(30),
        supabase.from("entity_relations").select("id, relation_type, weight, source_entity_id")
          .eq("target_entity_id", ent.id).order("weight", { ascending: false }).limit(30),
        supabase.from("entity_topics").select("topic_id").eq("entity_id", ent.id),
      ]);
      ...
      if (relEntityIds.length > 0) {
        const { data: relEntities } = await supabase
          .from("entities").select("id, entity_type, slug, title, summary")
          .in("id", [...new Set(relEntityIds)]).eq("is_published", true);
      ...
      const topicIds = (topicsRes.data || []).map((tp: any) => tp.topic_id);
      if (topicIds.length > 0) {
        const { data: topicData } = await supabase.from("topics").select("id, slug, title").in("id", topicIds);
      ...
      <div className="min-h-screen bg-background">
        <div className="max-w-3xl mx-auto px-4 py-20 text-center">
          ...
          <Link to={`/${entityType}`} className="text-sm text-primary hover:underline mt-4 inline-block">
            {t("entity_detail.back_to", { type: entityType })}
          </Link>
        </div>
      </div>
    );
  ...
  if (citations.length > 0) {
    jsonLd.citation = citations.map((c: any) => ({
      "@type": "CreativeWork", name: c.title || "Source",
      ...(c.url && { url: c.url }),
          ...
          <div className="space-y-10">
                  ...
                  {citations.map((c: any, i: number) => (
                    <div key={i} className="flex items-start gap-3 p-3 bg-card border border-border rounded-lg">
                      ...
                      <div className="min-w-0">
                        ...
                        {c.speaker && <p className="text-[10px] text-muted-foreground">{t("entity_detail.speaker", { name: c.speaker })}</p>}
                        {c.timestamp && <p className="text-[10px] text-muted-foreground">{t("entity_detail.timestamp", { value: c.timestamp })}</p>}
                ...
                <div className="flex flex-wrap gap-2">
                  {topics.map((tp) => (
                    <Link key={tp.id} to={`/topics/${tp.slug}`}
                      ...
                      <Tag className="h-3 w-3" /> {tp.title}
                    </Link>
                  ))}
                </div>
              </section>
            ...
            {Object.entries(relatedByType).map(([type, entities]) => (
              <section key={type}>
                ...
                <div className="space-y-2">
                  {entities.map((rel) => (
                    <Link key={`${rel.id}-${rel.relation_type}-${rel.direction}`}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate group-hover:text-primary transition-colors">{rel.title}</p>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[10px] text-muted-foreground">
                            {rel.direction === "incoming" ? "← " : "→ "}
                            {RELATION_LABEL[rel.relation_type] || rel.relation_type}
                          </span>
                          {rel.weight > 0 && <span className="text-[10px] text-muted-foreground/50">w:{rel.weight.toFixed(1)}</span>}
                        </div>
                        {rel.summary && <p className="text-[10px] text-muted-foreground truncate mt-0.5">{rel.summary}</p>}
                      </div>
            ...
            {related.length === 0 && topics.length === 0 && !entity.description && citations.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground">{t("entity_detail.enriching")}</p>
              </div>
            )}
          </div>
          ...
          <aside className="space-y-6">
            <div className="bg-card border border-border rounded-lg p-4 space-y-4">
              <div className="flex items-center gap-2 text-xs font-semibold text-foreground">
                <BarChart3 className="h-3.5 w-3.5" /> {t("entity_detail.idearank_scores")}
              </div>
              <ScoreBar label={t("entity_detail.idearank")} value={entity.idea_rank * 1000} max={10} />
              <ScoreBar label={t("entity_detail.importance")} value={entity.importance_score} />
              <ScoreBar label={t("entity_detail.confidence")} value={entity.confidence_score * 100} />
              <div className="pt-2 border-t border-border space-y-1.5 text-[10px] text-muted-foreground">
                <div className="flex justify-between">
                  <span>{t("entity_detail.evidence_count")}</span>
                  <span className="font-mono text-foreground">{entity.evidence_count}</span>
                </div>
                <div className="flex justify-between">
                  <span>{t("entity_detail.reuse_count")}</span>
                  <span className="font-mono text-foreground">{entity.reuse_count}</span>
                </div>
                <div className="flex justify-between">
                  <span>{t("entity_detail.graph_density")}</span>
                  <span className="font-mono text-foreground">{t("entity_detail.links", { count: graphDensity })}</span>
                </div>
              </div>
            </div>
          </aside>
