      ...
      <ContentBoundary width="narrow" className="relative text-center">
        ...
        <FadeInView delay={0.15}>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-12">
            <Button size="lg" onClick={ctaAction} className="cta-canon cta-glow gap-2.5 px-10 h-12 sm:h-14 bg-[hsl(var(--gold-oxide))] hover:bg-[hsl(var(--gold-dim))] text-[hsl(var(--obsidian))] font-semibold shadow-lg shadow-[hsl(var(--gold-oxide)/0.18)] w-full sm:w-auto rounded-lg transition-all duration-200">
              {t("final_cta.cta_start")}
              <ArrowRight className="h-4 w-4" />
            </Button>
            <Button variant="outline" size="lg" onClick={() => document.querySelector("#mechanism")?.scrollIntoView({ behavior: "smooth" })} className="cta-canon gap-2.5 h-12 sm:h-14 w-full sm:w-auto border-border/50 text-muted-foreground hover:bg-accent/8 hover:border-[hsl(var(--gold-oxide)/0.3)] rounded-lg transition-all duration-200">
