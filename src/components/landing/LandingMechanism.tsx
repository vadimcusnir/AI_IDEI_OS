...
export function LandingMechanism() {
  ...
  return (
    <section id="mechanism" className="py-28 sm:py-40 border-y border-border/50" aria-labelledby="mechanism-heading">
      <div className="max-w-5xl mx-auto px-5 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-border/50 rounded-xl overflow-hidden mb-24">
          {steps.map((step, i) => {
            ...
            return (
              <FadeInView key={step.num} delay={i * 0.08} className="bg-card p-8 sm:p-10 group hover:bg-accent/5 transition-colors">
                <div className="flex items-center justify-between mb-8">
                  <span className="text-4xl font-mono font-bold text-[hsl(var(--gold-oxide)/0.1)] group-hover:text-[hsl(var(--gold-oxide)/0.22)] transition-colors leading-none">{step.num}</span>
        ...
        <div className="space-y-1 max-w-3xl mx-auto">
          {transforms.map((t_item, i) => (
            <FadeInView key={i} delay={i * 0.05} className="flex items-center gap-6 py-4 px-5 rounded-lg hover:bg-accent/5 transition-colors">
