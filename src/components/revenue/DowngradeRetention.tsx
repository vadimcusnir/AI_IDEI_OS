...
export function DowngradeRetention({ open, onClose, onConfirmCancel, currentTier }: DowngradeRetentionProps) {
  ...
  useEffect(() => {
    ...
    async function loadStats() {
      setLoading(true);
      try {
        const jobsQ = supabase.from("neuron_jobs").select("id", { count: "exact", head: true }).eq("user_id", user!.id);
        const artifactsQ = supabase.from("artifacts").select("id", { count: "exact", head: true }).eq("author_id", user!.id);
        const neuronsQ = supabase.from("neurons").select("id", { count: "exact", head: true }).eq("author_id", user!.id);
        const creditsQ = supabase.from("credit_transactions").select("amount").eq("user_id", user!.id);
