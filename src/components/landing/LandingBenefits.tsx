    ...
    <section className="py-32 sm:py-44 border-y border-border/50" aria-label="Benefits">
      <ContentBoundary width="default">
        <FadeInView className="mb-20 sm:mb-24">
          <span className="text-eyebrow font-mono tracking-[0.3em] text-[hsl(var(--gold-oxide))] mb-6 block">{t("benefits.label")}</span>
          ...
          {items.map((b, i) => {
            ...
            return (
              <FadeInView
                key={i}
                delay={i * 0.1}
                className="relative p-6 sm:p-8 rounded-xl border border-border/50 bg-card hover:border-[hsl(var(--gold-oxide)/0.22)] landing-card group transition-all"
              >
