    ...
    <section className="py-32 sm:py-44 border-y border-border/50" aria-label="Who this is for">
      <div className="max-w-5xl mx-auto px-5 sm:px-6">
        <FadeInView>
          ...
          <p className="text-[15px] text-muted-foreground max-w-[480px] mb-16 sm:mb-24 leading-[1.75]">{t("who_for.subtitle")}</p>
        </FadeInView>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {roles.map((role, i) => {
            ...
            return (
              <FadeInView key={i} delay={i * 0.07} className="group flex items-start gap-4 p-6 sm:p-7 rounded-xl border border-border/50 bg-card hover:border-[hsl(var(--gold-oxide)/0.25)] landing-card min-h-[44px] transition-all">
        ...
        <FadeInView className="text-sm text-muted-foreground italic font-mono mt-14 sm:mt-16 border-t border-border/30 pt-7">
          {t("who_for.footnote")}
        </FadeInView>
      </div>
    </section>
