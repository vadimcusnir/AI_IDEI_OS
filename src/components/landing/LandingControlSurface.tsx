    ...
    <section id="control" className="py-32 sm:py-44" aria-labelledby="control-heading">
      <div className="max-w-4xl mx-auto px-5 sm:px-6">
        <FadeInView className="text-center mb-20 sm:mb-28">
          ...
          <p className="text-[15px] text-muted-foreground max-w-[480px] mx-auto leading-[1.75]">{t("control.subtitle")}</p>
        </FadeInView>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((ctrl, i) => (
            <FadeInView key={i} delay={i * 0.07} className="p-6 sm:p-7 rounded-xl border border-border/50 bg-card hover:border-[hsl(var(--gold-oxide)/0.25)] transition-all group flex items-start gap-4 sm:block landing-card min-h-[44px]">
              <IconControl className="text-[hsl(var(--gold-oxide))] mb-0 sm:mb-5 mt-0.5 sm:mt-0 shrink-0 group-hover:scale-110 transition-transform duration-300" size={20} />
              <div>
