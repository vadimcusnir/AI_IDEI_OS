...
export function useOSSuperlayer() {
  ...
  const load = useCallback(async () => {
    ...
    const [otosR, mmsR, lcssR, agentsR, execR, patR, unlR] = await Promise.all([
      ...
      supabase.from("os_memory_patterns").select("id, pattern_type, category, frequency, effectiveness_score, last_used_at, created_at, pattern_data").eq("user_id", user.id).order("frequency", { ascending: false }).limit(50),
